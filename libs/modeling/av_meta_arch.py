...
@register_meta_arch("AVLocPointTransformer")
class AVPtTransformer(nn.Module):
    """
    ...
    def forward(self, video_list):
        ...
        # pred_offsets = torch.cat(out_offsets, dim=1)
        # pred_clses = torch.cat(out_cls_logits,dim=1)
        # # left = pred_offsets[0,:,0].cpu().numpy()
        # # right = pred_offsets[0,:,1].cpu().numpy()
        # pred = torch.cat([pred_clses,pred_offsets],dim=-1)
        # pred = pred[0].cpu().numpy()
        # # pred_off = pred_offsets[0].cpu().numpy()
        # # pred_cls = pred_clses[0].cpu().numpy()
        # import numpy as np
        # import pandas as pd
        # import matplotlib.pyplot as plt
        # import seaborn as sns
        # sns.displot(data=pred,kind='kde')
        # sns.displot(data=right,kind='kde')
        ...
        if self.training:
            ...
            #     else:
            #         gt_segments.append(torch.tensor([]).to(self.device))
            #         gt_labels.append(torch.tensor(0).to(self.device))
            #         # gt_video_labels.append(torch.zeros(1).to(self.device))
            #     # gt_av_labels.append(video['av_labels'].to(self.device))
